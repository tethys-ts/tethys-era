...
# from tethys_cdsapi import utils, virtual_parameters
import utils, virtual_parameters
...
def preprocess_data_structure(nc_path, variable):
    """
    ...
    new_file_name = new_file_name_str.format(var=variable, date=time1_str)
    new_path = base_path.joinpath(new_file_name)
    xr1.to_netcdf(new_path, unlimited_dims=['time'])
...
class ERA5(tu.Grid):
    ...
    param_file_mappings = utils.param_file_mappings


    def variable_processing(self, nc_path, max_workers=4):
        """

        """
        if not isinstance(nc_path, str):
            raise TypeError('nc_path must be a glob string.')
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
            futures = []
            for v in variables:
                p1 = os.path.join(nc_path, self.param_file_mappings[v])
                p = glob.glob(p1)
                for nc_path in p:
                    f = executor.submit(preprocess_data_structure, nc_path, v)
                    futures.append(f)
